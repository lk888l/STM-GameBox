...
/*---------------------------中断配置-----------------------------------*/
	...
	//GPIO_B(4)外部中断的定义
	NVIC_InitTypeDef NVIC_InitStructure_B4;
	NVIC_InitStructure_B4.NVIC_IRQChannel = EXTI4_IRQn;
	NVIC_InitStructure_B4.NVIC_IRQChannelCmd = ENABLE;
	NVIC_InitStructure_B4.NVIC_IRQChannelPreemptionPriority = 1;
	NVIC_InitStructure_B4.NVIC_IRQChannelSubPriority = 1;
	NVIC_Init(&NVIC_InitStructure_B4);
	
	//GPIO_B(6)外部中断的定义
	NVIC_InitTypeDef NVIC_InitStructure_B;
	NVIC_InitStructure_B.NVIC_IRQChannel = EXTI9_5_IRQn;
...
{
	if (EXTI_GetITStatus(EXTI_Line6) == SET) //返回键  pB6
	{
...
{
	if (EXTI_GetITStatus(EXTI_Line4) == SET) //确认键  PB4
	{
	/*如果出现数据乱跳的现象，可再次判断引脚电平，以避免抖动*/
