...
#define STORE_START_ADDRESS		0x0800FFD0		//存储的起始地址
#define STORE_COUNT				24				//存储数据的个数
...
void Store_Init(void)
{
	...
	if (MyFLASH_ReadHalfWord(STORE_START_ADDRESS) != 0xA5A5)	//读取第一个半字的标志位，if成立，则执行第一次使用的初始化
	{
		...
		for (uint16_t i = 4; i < STORE_COUNT; i ++)				//循环STORE_COUNT次，除了第一个标志位
		{
			MyFLASH_ProgramHalfWord(STORE_START_ADDRESS + i * 2, 0x0000);		//除了标志位的有效数据全部清0
		}
		Store_Data[1] = 0x0001;							//利用flash记录光标状态
		Store_Data[2] = 0x0001;							//利用flash记录动画速度状态
		Store_Data[3] = 0x0001;							//利用flash记录静音状态
		MyFLASH_ProgramHalfWord(STORE_START_ADDRESS + 2, 0x0001);		//Store_Data[1]（0x0800FC02）写入0x0001
		MyFLASH_ProgramHalfWord(STORE_START_ADDRESS + 4, 0x0001);		//Store_Data[2]（0x0800FC04）写入0x0001
		MyFLASH_ProgramHalfWord(STORE_START_ADDRESS + 6, 0x0001);		//Store_Data[3]（0x0800FC06）写入0x0001
	}
	...
	/*上电时获取记录的状态*/	
	if(Store_Data[3] == 0x0001)			Buzzer_flag = 1;			//获取静音状态
	else if(Store_Data[3] == 0x0002) 	Buzzer_flag = 0;
	if(Store_Data[2] == 0x0001)										//获取动画速度状态
	{
		Speed_Factor = 8;
		Roll_Speed = 8;
	}
