...
// DMA alternates a GPIO set and reset write on every TIM3 update. This turns
...
extern "C" HAL_StatusTypeDef GameBox_Buzzer_Start(const std::uint16_t frequency_hz)
{
    ...
    GameBox_Buzzer_Stop();
    __HAL_TIM_SET_AUTORELOAD(&htim3, half_period - 1U);
    ...
    if (HAL_DMA_Start(&hdma_tim3_up, source, destination, 2U) != HAL_OK) {
        return HAL_ERROR;
    }
...
extern "C" void GameBox_Buzzer_Stop(void)
{
    ...
    if (hdma_tim3_up.State == HAL_DMA_STATE_BUSY) {
        (void)HAL_DMA_Abort(&hdma_tim3_up);
    }
    HAL_GPIO_WritePin(BUZZER_GPIO_Port, BUZZER_Pin, GPIO_PIN_RESET);
}
