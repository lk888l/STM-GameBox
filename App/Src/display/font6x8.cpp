...
// Adapted from the MIT-licensed STM-GameBox font table. See THIRD_PARTY_NOTICES.md.
constexpr std::uint8_t kGlyphs[][kFontGlyphWidth] = {
    ...
    {0x00,0x36,0x49,0x55,0x22,0x50}, {0x00,0x00,0x00,0x07,0x00,0x00},
    ...
    {0x00,0x3E,0x49,0x55,0x59,0x2E}, {0x00,0x7C,0x12,0x11,0x12,0x7C},
    ...
    {0x00,0x44,0x64,0x54,0x4C,0x44}, {0x00,0x00,0x08,0x7F,0x41,0x00},
    {0x00,0x00,0x00,0x7F,0x00,0x00}, {0x00,0x00,0x41,0x7F,0x08,0x00},
    {0x00,0x08,0x04,0x08,0x10,0x08},
};

static_assert((sizeof(kGlyphs) / sizeof(kGlyphs[0])) ==
              (kFontLastCharacter - kFontFirstCharacter + 1U));

} // namespace

const std::uint8_t* glyph6x8(char character)
{
    const auto value = static_cast<unsigned char>(character);
    const std::uint8_t printable =
        (value >= kFontFirstCharacter && value <= kFontLastCharacter)
            ? static_cast<std::uint8_t>(value)
            : static_cast<std::uint8_t>('?');
    return kGlyphs[static_cast<std::size_t>(printable - kFontFirstCharacter)];
}
