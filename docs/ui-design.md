...
旧菜单使用阻塞循环和“逐帧靠近目标”的动画，速度依赖循环执行时间。新 UI 是单一状态机，所有页面每帧由状态完整重绘；Canvas 和 shadow 的最终差异决定是否启动 OLED 传输。
...
菜单动画移植自 `refactor/rust-embassy`：`Spring` 使用 Q24.8 定点位置、速度和目标值，不使用 FPU。`MotionClock` 根据单调毫秒时间，每 8 ms 推进一次固定积分步；普通帧保留不足一步的时间余数，一帧最多补 8 步，长时间停顿后丢弃更多积压。SPI 的 5 ms 和 I2C 的 33 ms 渲染节拍因此保持相同动画速度，渲染抖动也不会改变弹簧参数。
...
默认 SPI 的 UI 目标周期为 5 ms；9 MHz 下 1024 B 纯数据传输约 0.91 ms，变化帧以一次完整数据 DMA 发送。I2C 的目标周期为 33 ms；400 kHz 下含页协议的全屏传输约 24 ms，并保留 128 B 差分页刷新。这些是调度目标和线速估算，并非本轮真机实测。静态画面经 shadow 比较后不产生传输。

运行期 OLED 数据和命令都通过 TX DMA 发送，UI 等待完成信号量；SPI 的末位 BSY 等待也在可阻塞的任务上下文完成。每轮从真实帧开始时间安排下一次唤醒，超过周期后丢弃旧节拍并至少阻塞 1 tick，因此没有 DMA 的静态画面也不会形成忙循环。输入任务优先级高于 UI，并独立以 5 ms 采样。

主页 RTC 只以 4 Hz 读取几个寄存器，秒数字实际每秒变化一次。I2C 的 shadow 差分通常只需更新顶栏涉及的页面；SPI 在画面变化时发送完整帧。Pet 在 Full/Reduced 下分别以 4/2 FPS 移动，Motion Off 时静止。这些页面动画沿用统一画布循环，不增加任务、堆或中断。

画布的矩形填充、横纵线与 6×8 字形都直接处理 SSD1306 原生页字节。未对齐字符最多写相邻两页，并以掩码保留字符范围外的像素；屏幕边界、反色、背景擦除及 dirty 标志由像素参考实现逐项回归验证。
