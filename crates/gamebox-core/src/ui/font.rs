//! Compact printable-ASCII 6×8 bitmap font.
...
pub(super) fn glyph(character: u8) -> &'static [u8; 6] {
    let printable = if (32..=126).contains(&character) {
        character
    } else {
        b'?'
    };
    &GLYPHS[usize::from(printable - 32)]
}
...
mod tests {
    ...
    #[test]
    fn printable_ascii_and_fallback_are_bounded() {
        assert_eq!(glyph(b' ')[0], 0);
